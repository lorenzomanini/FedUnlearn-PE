...
from fisherunlearn.information.spectral_wip import make_block_hessian_matvec, make_hessian_matvec
...
def main():
    ...
    parser.add_argument("--backend", choices=("full", "block"), default="full")
    parser.add_argument("--image-size", type=int, default=64)
    args = parser.parse_args()
    if min(args.batch_size, args.rank, args.hmp_chunk_size, args.image_size) < 1:
    ...
    if args.backend == 'block':
        model = extend(model, use_converter=True)
    dataset = TensorDataset(
        torch.randn(args.batch_size, 3, args.image_size, args.image_size),
        torch.randint(10, (args.batch_size,)),
    )
    factory = make_hessian_matvec if args.backend == 'full' else make_block_hessian_matvec
    matmat, _, _, n_params = factory(
    ...
    print(
        f"ResNet18 HMP: backend={args.backend}, device={device}, batch={args.batch_size}, "
